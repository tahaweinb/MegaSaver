...
Instagram / TikTok: instant best quality download
"""
...
YT_QUALITIES = [
    ...
    (
        "q_720",
        "🎥  720p",
        "bestvideo[height<=720]+bestaudio/bestvideo[height<=720]/best",
    ),
    (
        "q_480",
        "📱  480p",
        "bestvideo[height<=480]+bestaudio/bestvideo[height<=480]/best",
    ),
    (
        "q_360",
        "💾  360p",
        "bestvideo[height<=360]+bestaudio/bestvideo[height<=360]/best",
    ),
    (
        "audio",
        "🎵  Audio Only  MP3",
        "audio",  # special case handled separately
    ),
]

QUALITY_MAP: dict[str, tuple[str, str]] = {
    key: (label, fmt) for key, label, fmt in YT_QUALITIES
}
...
async def download(url: str, is_audio: bool, fmt: str, out_dir: Path) -> Path:
    ...
    if proc.returncode != 0:
        raise RuntimeError(stderr.decode().strip() or "yt-dlp exited with no output")
...
def friendly_error(err: str) -> str:
    e = err.lower()
    if "50m" in err or "file size" in e or "maxfilesize" in e:
    ...
    if "unavailable" in e or "removed" in e or "not available" in e:
        return "❌ This video is unavailable or has been removed."
    if "copyright" in e:
        return "⚠️ This video is blocked due to copyright restrictions."
    return "❌ Download failed. Please try again or use a different link."
...
async def handle_link(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Instagram / TikTok → instant download
    ...
    try:
        file_path = await download(url, False, "bestvideo+bestaudio/best", user_dir)
        size_mb   = file_path.stat().st_size / (1024 * 1024)
        caption   = (
            f"{emoji} *{platform.capitalize()}* — Best Quality\n"
            f"📦 {size_mb:.1f} MB  •  🤖 @megasaveroriginalbot"
        )
        await msg.edit_text("✅ *Done! Sending…*", parse_mode=ParseMode.MARKDOWN)
        await send_file(update.message, file_path, False, caption)
        await msg.delete()

    except Exception as exc:
        log.error("Download error: %s", exc)
        await msg.edit_text(friendly_error(str(exc)), parse_mode=ParseMode.MARKDOWN)
...
async def handle_quality(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        file_path = await download(url, is_audio, fmt, user_dir)
        size_mb   = file_path.stat().st_size / (1024 * 1024)
        caption   = (
            f"▶️ *YouTube* — {label}\n"
            f"📦 {size_mb:.1f} MB  •  🤖 @megasaveroriginalbot"
        ...
        log.error("Download error: %s", exc)
        await query.edit_message_text(friendly_error(str(exc)), parse_mode=ParseMode.MARKDOWN)
