...
YT_QUALITIES = [
    ...
    (
        "q_480",
        "📱  480p",
        ["-f", "bestvideo*[height<=480]+bestaudio/best", "--merge-output-format", "mp4"],
    ),
    (
        "q_360",
        "💾  360p",
        ["-f", "bestvideo*[height<=360]+bestaudio/best", "--merge-output-format", "mp4"],
    ),
    (
        "audio",
        "🎵  Audio Only  MP3",
...
INSTANT_FORMAT = ["-f", "best", "--merge-output-format", "mp4"]
...
def friendly_error(err: str) -> str:
    ...
    if "50m" in err or "file size" in e or "maxfilesize" in e:
        return "⚠️ File exceeds *50 MB*. Try a lower quality or Audio Only."
    if "copyright" in e:
...
async def cmd_help(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "▶️ *YouTube* — ✨ Best / 🎥 720p / 📱 480p / 💾 360p / 🎵 MP3\n"
